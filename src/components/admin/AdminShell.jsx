  ...
  return (
    <div className="min-h-screen bg-stone-100 flex">
      ...
      <aside className="w-60 shrink-0 bg-stone-900 text-stone-300 flex flex-col fixed inset-y-0 left-0 z-30">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 pr-2 space-y-1 sidebar-nav-scroll">
          {nav.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
      ...
      <div className="flex-1 ml-60 flex flex-col min-w-0">
        <header className="sticky top-0 z-20 bg-white border-b border-stone-200 px-6 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-2 text-sm text-stone-500">
            <span className="inline-flex items-center rounded-full bg-stone-100 px-3 py-1.5 font-medium text-stone-700">
