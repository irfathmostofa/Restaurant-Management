    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-4 border-t border-stone-800 space-y-3">
        <div className="text-sm">
          ...
          {activeBranch && <div className="text-xs text-brand-400 mt-1 truncate">{activeBranch.name}</div>}
        </div>
        <button
          onClick={() => { onNavigate(); navigate('/admin/profile') }}
...
export default function AdminShell() {
  ...
  const { activeBranch, activeBranchId, setActiveBranchId, canSwitchBranches, branches } = useBranch()
  ...
  return (
    <div className="min-h-screen bg-stone-100 flex">
      ...
      <aside className="hidden lg:flex w-64 shrink-0 bg-stone-900 text-stone-300 flex-col fixed inset-y-0 left-0 z-30">
        <SidebarContent staff={staff} nav={nav} activeBranch={activeBranch} navigate={navigate} onNavigate={() => {}} />
      </aside>
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-w-0">
        <header className="sticky top-0 z-20 bg-white border-b border-stone-200 px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <button onClick={() => navigate(homeRoute)} className="text-sm text-stone-500 hover:text-brand-600 truncate">
              {homeLabel}
            </button>
          </div>
          <div className="flex items-center gap-3 min-w-0">
            {canSwitchBranches ? (
              ...
              <span className="text-sm font-medium text-stone-600 truncate">Branch: {activeBranch.name}</span>
            ) : null}
            <NavLink
              to="/admin/profile"
            ...
            >
              {(staff.profile_image_url)
                ? <img src={staff.profile_image_url} alt={staff.name} className="w-9 h-9 rounded-full object-cover" />
