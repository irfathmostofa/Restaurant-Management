...
export default function Billing() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase
        ...
        .or(
          `status.in.(received,preparing,ready,served),created_at.gt.${cutoff}`,
        )
      ...
      const methods = (bpmRes.data || [])
        .map((r) => r.payment_methods)
      ...
      fetchEffectiveTaxSettings(activeBranchId).then((ts) => {
        if (active) setTaxSettings(ts);
      });
      setLoading(false);
    });
    return () => {
        ...
        (payload) => {
          setItemsByOrder((prev) => {
            ...
            if (payload.eventType === "DELETE") {
              ...
              next[payload.new.order_id] = [
                ...list.filter((it) => it.id !== payload.new.id),
                payload.new,
              ];
            }
            return next;
        ...
        (payload) => {
          setPaymentsByOrder((prev) => ({
            ...
            [payload.new.order_id]: [
              ...(prev[payload.new.order_id] || []).filter(
  ...
  if (!activeBranch)
    return <p className="text-stone-500">Select a branch to bill orders.</p>;

  const total = (id) =>
    (itemsByOrder[id] || []).reduce(
      (s, it) => s + Number(it.price_at_order) * it.quantity,
      0,
    );
  ...
  const { vat, tax, serviceCharge, grandTotal } = computeTotals({
    subtotal,
    discount: disc,
    taxSettings,
  });
  ...
  const processPayment = async () => {
    ...
    if (order.status === "paid" || order.status === "cancelled") {
      setError(
        `This order is already ${order.status} and cannot be charged again.`,
      );
      setPayingOrder(null);
      return;
    }
    ...
    if (orderError) {
      ...
      if (win) win.close();
      setError(
        "Payment recorded, but the order could not be updated. Please refresh.",
      );
      setPaying(false);
      return;
    }
    if (order?.table_id) {
      ...
      if (tableError)
        console.error(
          "Failed to free table after payment:",
          tableError.message,
        );
    }
  ...
  const reprintInvoice = async (order) => {
    ...
    const html = buildInvoiceHtml({
      ...
      branch: activeBranch,
      invoiceNo:
        payment.invoice_no || `INV-${order.id.slice(0, 8).toUpperCase()}`,
      orderNo: order.id.slice(0, 8).toUpperCase(),
      orderTime: order.created_at,
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={
          activeBranch
            ? `Collect payment at ${activeBranch.name}`
            : "Select a branch"
        }
        actions={
          <button
      ...
      {loading ? (
        ...
        <EmptyState
          message="No unpaid orders."
          hint="Orders sent to the kitchen will appear here."
        />
      ) : (
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
          {(showPaid ? orders : billable).map((o) => {
            const eta = hasKitchenItems(itemsByOrder[o.id])
              ? orderKitchenEta(itemsByOrder[o.id])
              : null;
            return (
              <div
                key={o.id}
                className={`bg-white rounded-xl border p-5 ${o.status === "paid" ? "border-stone-200 opacity-70" : "border-stone-200"}`}
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="text-right">
                    <span className="text-xs font-medium rounded-full px-2.5 py-0.5 capitalize bg-stone-100 text-stone-600">
                      {o.type}
                    </span>
                    <div className="text-xs text-stone-400 mt-1 capitalize">
                      {o.status}
                    </div>
                  </div>
                </div>
                {eta !== null && o.status !== "paid" && (
                  <div className="mb-2">
                    <span
                      className={`inline-block text-xs font-medium rounded-full px-2.5 py-0.5 ${eta > 0 ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}
                    >
                ...
                <div className="flex items-center justify-between pt-3 border-t border-stone-100">
                  <span className="font-semibold text-stone-900">
                    {formatMoney(total(o.id))}
                  </span>
                  <div className="flex items-center gap-2">
                    {o.status === "paid" ? (
                      <button
                        onClick={() => reprintInvoice(o)}
                        className="px-3 py-1.5 rounded-lg border border-stone-300 text-stone-600 text-sm font-medium hover:bg-stone-50"
                      >
                        Reprint
                      </button>
                    ) : (
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
              ...
              {paymentMethods.length === 0 ? (
                ...
                <div className="grid grid-cols-2 gap-2">
                  {paymentMethods.map((m) => (
                    <label
                      key={m.id}
                      className={`flex items-center gap-2 p-3 rounded-lg border cursor-pointer ${payMethodId === m.id ? "border-brand-400 bg-brand-50" : "border-stone-200 hover:border-brand-400"}`}
                    >
                      <input
                        type="radio"
                        name="method"
                        value={m.id}
                        checked={payMethodId === m.id}
                        onChange={() => setPayMethodId(m.id)}
            ...
            <div className="space-y-1 mb-4 text-sm">
              ...
              {vat > 0 && (
                <div className="flex justify-between text-stone-600">
                  <span>
                    {taxSettings.vat_name || "VAT"} ({taxSettings.vat_rate}%)
                  </span>
              ...
              {tax > 0 && (
                <div className="flex justify-between text-stone-600">
                  <span>
                    {taxSettings.tax_name || "Tax"} ({taxSettings.tax_rate}%)
                  </span>
            ...
            {isCash && (
              <div className="mb-4">
                ...
                <p className="text-sm text-stone-500 mt-1">
                  Change:{" "}
                  <b className="text-stone-800">{formatMoney(change)}</b>
                </p>
              </div>
            )}
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setPayingOrder(null)}
                className="flex-1 px-4 py-2 rounded-lg border border-stone-300 text-sm font-medium text-stone-600 hover:bg-stone-50"
              >
                Cancel
              </button>
              <button
