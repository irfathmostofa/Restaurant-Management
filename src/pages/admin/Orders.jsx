...
// Manual advance path for monitoring staff (the kitchen uses per-item controls).
const NEXT_STATUS = {
  received: 'preparing',
  preparing: 'ready',
  ready: 'served',
...
export default function Orders() {
  ...
  // Realtime: kitchen status / ETA changes on order_items
  useEffect(() => {
    if (!activeBranchId) return
    const channel = supabase
  ...
  const startPreparing = async (order) => {
    ...
    await updateKitchenItems(order.id, { kitchen_status: KITCHEN_STATUS.PREPARING, prep_started_at: new Date().toISOString() })
  ...
  const addPrepMinutes = async (order, minutes) => {
    ...
    for (const it of items || []) {
      await supabase.from('order_items').update({ estimated_prep_time: Number(it.estimated_prep_time) + minutes }).eq('id', it.id)
    }
    setBusy((b) => ({ ...b, [order.id]: null }))
  }
  ...
  const markReady = async (order) => {
    ...
    await updateKitchenItems(order.id, { kitchen_status: KITCHEN_STATUS.READY })
  ...
  // Kitchen staff only see orders that actually require kitchen prep.
  const displayed = isKitchen
    ? orders
        .filter((o) => !['paid', 'cancelled'].includes(o.status))
        .filter((o) => hasKitchenItems(itemsByOrder[o.id]))
        .sort((a, b) => new Date(a.created_at) - new Date(b.created_at))
    : orders.filter((o) => (showPaid ? true : !['paid', 'cancelled'].includes(o.status)))
  ...
  const KitchenTicket = ({ order }) => {
    ...
    return (
      <div className={`bg-white rounded-xl border p-5 ${isReady ? 'border-emerald-200' : 'border-stone-200'}`}>
        <div className="flex items-start justify-between mb-2">
          <div>
            <span className="text-xs uppercase tracking-wide text-stone-400">Order</span>
            <div className="font-bold text-stone-900 text-lg">#{order.id.slice(0, 8).toUpperCase()}</div>
          </div>
          <div className="flex flex-col items-end gap-1">
            <span className={`text-xs font-medium rounded-full px-2.5 py-1 ${KITCHEN_STATUS_STYLE[status]}`}>
              {KITCHEN_STATUS_LABELS[status]}
            </span>
            <span className="text-xs text-stone-400 capitalize">{order.type}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mb-3 text-sm">
          <div>
            ...
            <span className="font-medium">{order.type === 'takeaway' ? 'Takeaway' : (tableNumber(order.table_id) || '—')}</span>
          </div>
          <div>
          ...
          {kitchenItems.map((it) => (
            <li key={it.id}>
              <div className="flex items-center justify-between gap-2">
                ...
                <span className={`text-[10px] font-medium rounded px-1.5 py-0.5 ${KITCHEN_STATUS_STYLE[it.kitchen_status]}`}>
                  {KITCHEN_STATUS_LABELS[it.kitchen_status]}
                </span>
              </div>
              {it.notes && <p className="text-xs text-stone-500 ml-4">• {it.notes}</p>}
            </li>
          ))}
        </ul>

        <div className="flex items-center justify-between pt-3 border-t border-stone-200 mb-3">
          <span className="text-xs text-stone-400">Estimated time</span>
          <span className={`text-lg font-bold ${isReady ? 'text-emerald-600' : 'text-stone-900'}`}>
            {isReady ? 'Ready' : `${eta} min`}
          </span>
        </div>
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <EmptyState message={isKitchen ? 'Kitchen queue is empty.' : 'No active orders.'} hint={isKitchen ? 'New orders appear here in real time.' : 'New orders appear here in real time.'} />
      ) : (
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
          {displayed.map((o) => (
            isKitchen ? <KitchenTicket key={o.id} order={o} /> : (
              <div key={o.id} className="bg-white rounded-xl border border-stone-200 p-5">
                ...
                <div className="mb-3">
                  ...
                  {hasKitchenItems(itemsByOrder[o.id]) && (
                    <span className="ml-2 text-xs text-stone-500">ETA {orderKitchenEta(itemsByOrder[o.id])} min</span>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-stone-100">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {!['served', 'paid', 'cancelled'].includes(o.status) && (
                      <button onClick={() => setStatus(o, 'cancelled')} className="text-xs text-red-500 hover:text-red-700">Cancel</button>
                    )}
                  </div>
                </div>
              </div>
