...
export default function Settings() {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([fetchSettings(), fetchAllRoleRoutes()]).then(
      ([settingsData, routesData]) => {
        if (!active) return;
  ...
  const save = async (e) => {
    ...
    const routeRows = Object.entries(ROLES).map(([, role]) => ({
      role,
      route: routes[role] || DEFAULT_ROUTE_BY_ROLE[role],
    }));
    ...
    logActivity({
      module: "settings",
      action: "update",
      description: "Updated restaurant settings and role landing pages",
    });
  ...
  const saveCurrency = async (e) => {
    ...
    logActivity({
      module: "currency",
      action: "update",
      description: `Updated currency to ${payload.name} (${payload.iso_code})`,
    });
    ...
    <div>
      ...
      <div className=" flex sm:flex-row flex-col items-start  gap-6">
        <form onSubmit={save} className="max-w-2xl space-y-6">
            ...
            <div className="space-y-4">
              <div>
                <ImageUploader
                  ...
                  onChange={(url) =>
                    setSettings({ ...settings, restaurant_logo: url })
                  }
                  bucket="branding"
                  folder="restaurant"
                  maxDimension={512}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSettings({ ...settings, invoice_footer: e.target.value })
                  }
                  className="w-full px-3 py-2 rounded-lg border border-stone-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSettings({
                      ...settings,
                      default_prep_time: e.target.value,
                    })
                  }
                  className="w-full px-3 py-2 rounded-lg border border-stone-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
                />
          ...
          <div className="bg-white rounded-xl border border-stone-200 p-6">
            ...
            <p className="text-sm text-stone-500 mb-4">
              Where each role lands right after login. Changes apply without
              touching app code.
            </p>
            <div className="space-y-3">
              {Object.values(ROLES).map((role) => (
                ...
                >
                  <span className="text-sm font-medium text-stone-700 w-40">
        ...
        <form
          ...
          className="max-w-2xl bg-white rounded-xl border border-stone-200 p-6 space-y-4"
        >
          <div>
            <h2 className="font-semibold text-stone-900">Currency</h2>
            <p className="text-sm text-stone-500">
              Controls how money is displayed across the website, POS, invoices
              and reports.
            </p>
          </div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                value={currencyForm.iso_code}
                onChange={(e) =>
                  setCurrencyForm({ ...currencyForm, iso_code: e.target.value })
                }
                placeholder="USD"
                maxLength={3}
                className="w-full px-3 py-2 rounded-lg border border-stone-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setCurrencyForm({
                    ...currencyForm,
                    symbol_position: e.target.value,
                  })
                }
                className="w-full px-3 py-2 rounded-lg border border-stone-300 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-500"
              >
            ...
            <div>
              ...
              <input
                value={currencyForm.thousand_separator}
                onChange={(e) =>
                  setCurrencyForm({
                    ...currencyForm,
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm text-stone-500 ml-auto">
              Preview: {formatCurrencyPreview(currencyForm, 1234.5)}
            </span>
          </div>
        </form>
      </div>
